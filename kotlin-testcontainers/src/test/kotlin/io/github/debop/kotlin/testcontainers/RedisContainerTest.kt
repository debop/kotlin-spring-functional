...
class RedisContainerTest {
    ...
    companion object: KLogging() {
        ...
        fun RedisClient.withCommands(block: RedisCommands<String, String>.() -> Unit) {
            connect().use {
                block.invoke(it.sync())
            }
        }

        fun RedisClient.withAsyncCommands(block: RedisAsyncCommands<String, String>.() -> Unit) {
        ...
        fun RedisClient.withReactiveCommands(block: RedisReactiveCommands<String, String>.() -> Unit) {
            connect().use {
                block.invoke(it.reactive())
            }
        }

        fun <T> RedisClient.withBatch(duration: Duration = Duration.ofSeconds(30),
    ...
    fun `execute in batch`() {
        ...
        val results = redisClient.withBatch(Duration.ofMinutes(1)) {
            List(batchSize) { set("batch-key-$it", "batch-value-$it") }.toList()
        }
